...
4. ADAPTABILIDAD - Recálculo automático al cambiar geocerca
5. PLACEHOLDER - Animales sin geocerca en primera geocerca disponible

CARACTERÍSTICAS TÉCNICAS:
========================
- Movimiento errático y natural dentro de límites
- Bouncing physics en los bordes (rebote natural)
- Un animal designado como "oveja negra" con probabilidad de escape
- Sistema de coordenadas completamente abstracto
- Compatible con cualquier geocerca del mundo
- Migración automática entre geocercas
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'🔄 Adaptabilidad dinámica a cambios de geocerca')
        self.stdout.write(self.style.SUCCESS('=' * 90 + '\n'))
                    ...
                    for animal_data in animales_data:
                        ...
                        try:
                            ...
                            if current_position:
                                ...
                                # Verificar si cambió la geocerca (adaptabilidad)
                                if await self.geofence_changed(collar_id, geofence_data['id']):
                                    self.stdout.write(
                                        self.style.SUCCESS(f'🔄 {display_id}: Geocerca cambiada - Reposicionando...')
                                    )
                            ...
                            self.stdout.write(
                                f'  {status_emoji} {display_id}{sheep_marker}: '
    ...
    @sync_to_async
    def geofence_changed(self, collar_id, current_geofence_id):
        """Verifica si la geocerca del animal cambió desde la última telemetría"""
        ...
        last_geofence_id = last_pos.animal.geocerca_id
        return last_geofence_id != current_geofence_id
    ...
    def random_walk_movement(self, lat, lng, polygon, movement_range):
        ...
        Este es el algoritmo principal de movimiento para animales normales.
        Características:
        - Movimiento completamente aleatorio en todas direcciones
        - Bouncing physics en los bordes (rebote natural)
        - Sin atracción al centroide
        - Movimiento errático y natural
        """
        # Generar movimiento aleatorio puro
        ...
        if polygon.contains(punto_nuevo):
            ...
            # BOUNCING PHYSICS - Rebote en el borde
            ...
            if polygon.contains(punto_rebote):
                ...
                # Si el rebote también falla, quedarse en posición actual
                # (esto solo pasa en geocercas muy pequeñas)
                return lat, lng
    ...
    def escape_movement(self, lat, lng, polygon, centroid, movement_range):
        ...
        Características:
        - Movimiento HACIA AFUERA (opuesto al centroide)
        - Mayor rango de movimiento
        - Objetivo: salir de la geocerca
        """
        # Calcular vector desde centroide hacia posición actual
        vector_lat = lat - centroid.y
        vector_lng = lng - centroid.x
        
        # Si está muy cerca del centroide, usar dirección aleatoria
        ...
        # Movimiento hacia afuera (amplificado)
        escape_factor = random.uniform(3, 5)  # 3-5x más rápido
    ...
    def continue_escape(self, lat, lng, polygon, centroid, movement_range):
        ...
        """
        # Continuar moviéndose lejos del centroide
        vector_lat = lat - centroid.y
        vector_lng = lng - centroid.x
        ...
        # Continuar alejándose con algo de aleatoriedad
        escape_factor = random.uniform(1.5, 2.5)
        delta_lat = vector_lat * movement_range * escape_factor
        delta_lng = vector_lng * movement_range * escape_factor
        
        # Agregar componente aleatorio para movimiento más natural
